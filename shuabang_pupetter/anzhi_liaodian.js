...
const launchBrowser = async () => {
    try {
        ...
        log("进入安智市场")
        await pages[1].waitForSelector("input[id=search_txt]");
        log("3333")
        log("搜索")
        let keyword1 = await pages[1].$("input[id=search_txt]");
        await keyword1.click();
        let Str1 = "社团";
        await keyword1.type(Str1, {delay: 50});
        log("点击搜索")
        const searchbtn1 = await pages[1].$("input[id=search_btn]");
        await searchbtn1.click();
        await sleep()
        // 等待数据出现，否则鲍报错
        await pages[1].waitForSelector(".content .app_icon");
        const page_4 = await pages[1].$$(".pagebars > a");
        await page_4[3].click();
        await sleep()
        await pages[1].waitForSelector(".content .app_icon");
        await pages[1].evaluate(() => {
            ...
            //     clearInterval(index)
            console.log("ddd")
            let apps = document.querySelectorAll(".app_list > ul > li");
            for (let app of apps) {
                let appname = app.querySelector(".app_name > a").textContent
                // console.log(appname)
        ...
        await sleep(5)
        log("进入下载")
        let liaodian = await pages[1].$(".app_list .app_name > a");
        ...
        // });
        // await sleep(2)
        await pages[1].waitForSelector(".detail_down > a");
        let download1 = await pages[1].$(".detail_down > a");
        await download1.click();
        let time = new Date();
        let obj = {download_count,time};
        await Queue.postDataToMessage("shuabang_anzhi_liaodian", JSON.stringify(obj));
