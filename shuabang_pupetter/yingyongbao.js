...
const launchBrowser = async () => {
    try {
        ...
        await pages[1].waitForSelector("div[class=com-header-center-container] .com-menu-btn");
        log("3333")
        ...
        await sleep()
        log("搜索社团")
        await pages[1].waitForSelector("input[id=J_MainInput");
        let keyword1 = await pages[1].$("input[id=J_MainInput]");
        await keyword1.click();
        let Str1 = "社团";
        await keyword1.type(Str1, {delay: 50});
        log("点击搜索")
        const searchbtn1 = await pages[1].$("input[id=J_SearchBtn]");
        await searchbtn1.click();
        await sleep()
        await pages[1].waitForSelector(".search-boutique-app-box .appName");
        await pages[1].evaluate(() => {
            ...
            setTimeout(() => {
                clearInterval(index)
                console.log("ddd")
                let apps = document.querySelectorAll(".search-default-list .search-boutique-app-box");
                for (let app of apps) {
                    let appname = app.querySelector(".appName").textContent
                    console.log(appname)
        ...
        let liaodian = await pages[1].$(".search-default-list .search-boutique-app-box .icon");
        log("555")
        ...
        await sleep()
        await pages[2].waitForSelector(".det-ins-btn-box .det-down-btn");
        const download1 = await pages[2].$(".det-ins-btn-box .det-down-btn");
        await download1.click();
        let time = new Date();
        let obj = {download_count,time};
        await Queue.postDataToMessage("shuabang_yingyongbao_liaodian", JSON.stringify(obj));
